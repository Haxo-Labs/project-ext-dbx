...
docker run -d --name dbx -p 8080:3000 \
  -e DBX_BACKEND_1_PROVIDER=redis \
-e DBX_BACKEND_1_URL=redis://user:pass@redis.com:6379 \
-e DBX_DEFAULT_BACKEND=backend_1 \
  -e PORT=3000 \
  -e LOG_LEVEL=DEBUG \
  effortlesslabs/0dbx_redis:latest
...
| ----------- | ------------------------ | ---------------------------------------- |
| `DBX_BACKEND_1_PROVIDER` | `redis` | Backend provider type |
| `DBX_BACKEND_1_URL` | `redis://localhost:6379` | Backend connection URL |
| `DBX_DEFAULT_BACKEND` | `backend_1` | Default backend to use |
